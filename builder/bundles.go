...
func extractRpm(baseDir string, rpm string) error {
	...
	err = cmd.Wait()
	if err != nil {
		fmt.Println(err.Error(), "rpm2cpio failed")
		return err
	}
	err = pw.Close()
	if err != nil {
	...
	err = cmd2.Wait()
	if err != nil {
		fmt.Println(err.Error(), "cpio failed")
		return err
	}
...
func installBundleToFull(packagerCmd []string, buildVersionDir string, bundle *bundle, downloadRetries int, numWorkers int, localPath string) error {
	...
	var errCh error
	var rpmList []string
	for rpm, present := range bundle.AllPackages {
		if present {
			if rpmMap[rpm] {
				continue
			}
			...
			if _, err = os.Stat(rpmFull); err != nil {
				rpm = strings.TrimSuffix(rpm, ".rpm")
	...
	if !Offline {
		if len(rpmList) > 0 {
			...
			for _, rpmOnline := range rpmList {
				rpmOnline = rpmOnline + ".rpm"
				if rpmMap[rpmOnline] {
					continue
				}
				rpmFull := filepath.Join(newPath[0], rpmOnline)
				if _, err = os.Stat(rpmFull); err != nil {
					rpmList = append(rpmList, rpmOnline)
					continue
				}
				rpmMap[rpmOnline] = true
				select {
				case rpmCh <- rpmFull:
				case errCh = <-errorCh:
					break
...
func buildFullChroot(b *Builder, set *bundleSet, packagerCmd []string, buildVersionDir, version string, downloadRetries int, numWorkers int) error {
	...
	err := os.MkdirAll(filepath.Join(buildVersionDir, "full"), 0755)
	if err != nil {
		return err
	}
	i := 0
